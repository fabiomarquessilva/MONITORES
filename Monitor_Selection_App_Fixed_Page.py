...
st.subheader("📘 Explicação da Fórmula")
st.write("""
...
cp = st.number_input("Créditos Práticos (Cp):", min_value=0, step=1)
num = st.number_input("Número de Alunos (Num):", min_value=0, step=1)
e = st.number_input("Número de Turmas (E):", min_value=1, step=1)
b = st.radio("Teve monitor no semestre anterior?", ["Sim", "Não"])
r = 20 if b == "Não" else 0
semestres_sem_monitor = st.number_input("Semestres consecutivos sem monitor:", min_value=0, step=1)
